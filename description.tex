...
\item[Free variables or functions] are \textcolor{green}{green}. This includes quantities such as $\fv{x}$ and $\fv{n}$ in the binomial approximation $(\fc{1}\op{+}\fv{x})^{\fv{n}} \approx \fc{1} \op{+} \fv{n}\fv{x}$
...
\item[Indices and other bound variables] are \bv{gray}. This includes $\bv{k}$ in $\op{\sum_{\bv{k} \nt{=} \fc{0}}^{\fc{100}}}$ and $\bv{x}$ in $\op{\int} \! \bv{x}^{\op{2}} \, \op{\mathrm{d}} \bv{x}$ as well as tensor indices.
