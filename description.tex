...
Not all books have a good glossary or index.
...
\definecolor{DarkGreen}{HTML}{006400} % svgnames DarkGreen
\definecolor{Plum}{HTML}{8d188f} % dvipsnames Plum
\definecolor{Gray}{HTML}{808080} % svgnames Gray

\newcommand{\op}{\textcolor{Plum}} % functions and operators
\newcommand{\fv}{\textcolor{DarkGreen}} % free variables
...
\begin{description}
  \item[Operators and special functions] are \op{\textbf{purple}}.
      ...
      If it acts on zero or more inputs and cannot be freely redefined,
      it falls into this category.
    \item[Free variables and free functions] are \fv{\textbf{green}}. This includes quantities such as $\fv{x}$ and $\fv{n}$ in the binomial approximation $(\fc{1}\op{+}\fv{x})^{\fv{n}} \approx \fc{1} \op{+} \fv{n}\fv{x}$
    ...
    \item[The independent variable] is \iv{\textbf{red}}.
    ...
    \item[Fixed constants] are \fc{\textbf{blue}}.
      These include numeric constants like $\fc{0}$, $\fc{\pi}$ or $\fc{e}$,
      vector constants like the unit vector $\fc{\hat{x}}$,
    ...
    \item[Indices and other bound variables] are \bv{\textbf{gray}}.
