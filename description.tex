...
as is the tendency towards returning the ``one true form'' of an equation.
...
It might possible to identify each equation in,
say, a \LaTeX\ document,
and automatically convert the units from,
say, cgs to SI.

In practice, however,
because of things like \LaTeX\ macros and multiple kinds of cgs
...
Equations that ``know'' how they are derived can explicitly show which assumptions they require.

It could make it trivial to answer questions like,
...
Feynman's might have seen something like this.
...
Functions, variables, constants, and indices could each have their own color.

Color-coding would provide an extra degree of freedom for distinguishing quantities
and dramatically increase the information density of an equation.
Furthermore, it would provide a kind of rudimentary visual type system for mathematics.
Adding a variable to an index would be obviously wrong,
for example.

Equation metadata would provide insight into what is invariant and fundamental in an equation
