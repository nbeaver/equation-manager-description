...
one could automatically color-code the expression.
Functions and operators,
independent free variables,
dependent free variables,
bound (a.ka. ``dummy'') variables or indices,
and constants could each have their own color.

Here is an example coloring scheme:

\begin{description}
\item[Well-defined functions and operators] are \textcolor{Brown}{brown}.
...
\item[Constants] are \textcolor{blue}{blue}.
      Some of these are numeric constants like $\pi$ or $e$,
      while others are physical constants like the speed of light $c$ or Avogadro's number $N_A$,
      and still others are constant sets like the set of real numbers $\mathbb{R}$. 
      Note that this does not include quantities that must be kept constant in the validity regime of the equation.
      For example,
      the physical relation described by Snell's Law
      $n_1 \sin(\theta_1) = n_2 \sin(\theta_2)$
      does not describe a system where $n_1$ varies with position
      (although in such a system Snell's Law can be integrated over the region to obtain the path of the ray).
      Nevertheless,
...
\item[Indices and other bound variables] are \textcolor{darkgray}{gray}. This includes $k$ in $\sum_{k = 0}^{100}$ and $x$ in $\int \! x^2 \, \mathrm{d}x$.
...
For example,
it is skewed in favor of equations used by physicists,
not equations used by e.g. statisticians or group theorists.

%TODO Color-code these equations.
...
\[
\oint_S \vec{E} \cdot d \vec{A} = \frac{Q_{in}}{\varepsilon_0}
\]
...
and dramatically increase the information density of an equation.
Furthermore, it is a kind of rudimentary visual type system for mathematics.
Adding a variable to an index is obviously wrong,
for example.
